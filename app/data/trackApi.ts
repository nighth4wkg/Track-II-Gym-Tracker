...
import { emptyPreviousSetMaps, fetchPreviousSetMaps, normalizedHistoryName } from "./progressionHistoryApi";
    ...
    tasks: (exercisesBySplit.get(split.id) ?? []).map((exercise) => ({
      ...
      sets: (setsByExercise.get(exercise.id) ?? []).map((set) => {
        ...
        return {
          ...
          historySessions: previous?.historySessions,
          historySamples: previous?.historySamples,
          historyFailureCount: previous?.historyFailureCount,
        };
      }),
    })),
  }));
