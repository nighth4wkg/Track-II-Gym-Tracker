...
const replaceBuildTokens = (value: string) => value.replaceAll("__TRACK_BUILD_ID__", buildId).replaceAll("__TRACK_VERSION__", releaseVersion);

export default defineConfig(({ mode }) => {
  const env = loadEnv(mode, process.cwd(), "");
  const supabaseUrl = env.NEXT_PUBLIC_SUPABASE_URL ?? "";
  const supabasePublishableKey = env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY ?? "";
  const trackWebOrigin = env.NEXT_PUBLIC_TRACK_WEB_ORIGIN ?? "";
  const trackReleasesUrl = env.NEXT_PUBLIC_TRACK_RELEASES_URL ?? "";
  const trackIssuesUrl = env.NEXT_PUBLIC_TRACK_ISSUES_URL ?? "";
  ...
  return {
    ...
    plugins: [react(), {
      ...
      generateBundle(_options, bundle: OutputBundle) {
        this.emitFile({ type: "asset", fileName: "track-release.json", source: `${JSON.stringify({ version: releaseVersion, buildId }, null, 2)}\n` });
        for (const asset of Object.values(bundle)) {
          if (asset.type !== "asset") continue;
          const source = asset.source instanceof Uint8Array ? new TextDecoder().decode(asset.source) : asset.source;
          asset.source = replaceBuildTokens(source);
        }
      },
    }],
    define: {
