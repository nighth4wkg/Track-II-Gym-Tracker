...
[Native production release](docs/native-production-release.md) ·
...
summary and revision RPCs. The following logical-session migration keeps raw
records intact while treating repeated finishes from the same split on the
same local calendar day as one dashboard session. Different splits remain
separate. The RPC uses the requested local timezone for period boundaries and
returns the dashboard's volume periods, progression feed, and weekly muscle
totals.
...
Production monitoring runs a low-volume, read-only web probe every 15 minutes
and checks the public site, release manifest, Supabase CORS, and safe auth
responses. Configure the production smoke secrets in the repository before
enabling it. The weekly backup workflow requires a server-only
`SUPABASE_DB_URL` repository secret, exports a custom-format PostgreSQL backup,
restores it into an isolated PostgreSQL service, and verifies the required
tables and RLS flags. It never restores over the production database.
...
The current release is [Track II v1.0.14](https://github.com/nighth4wkg/Track-II-Gym-Tracker/releases/tag/v1.0.14).
...
through the [v1.0.14 GitHub Release](https://github.com/nighth4wkg/Track-II-Gym-Tracker/releases/tag/v1.0.14).
The web bundle is uploaded to Cloudflare Pages, while native IPA/APK artifacts
are generated by the [Native Release workflow](https://github.com/nighth4wkg/Track-II-Gym-Tracker/actions/workflows/native-release.yml)
for unsigned testing or the protected [Native Production Release workflow](https://github.com/nighth4wkg/Track-II-Gym-Tracker/actions/workflows/native-production-release.yml)
