-- Track II: one owner-scoped dashboard read.
--
-- This migration is additive. It creates indexes/functions only; it never
-- rewrites or deletes workout data. Session ids are the aggregation identity.

create index if not exists workout_logs_owner_session_created_idx
  on public.workout_set_logs(user_id, session_id, created_at desc);

create or replace function public.get_dashboard_revision(time_zone text default 'UTC')
returns bigint
language sql
stable
security invoker
  ...
  select case
    when auth.uid() is null then 0::bigint
      ...
      hashtextextended(
        concat_ws(
          '|',
          coalesce((select revision::text from public.track_state_revisions where user_id = auth.uid()), '0'),
          coalesce((select count(*)::text from public.workout_sessions where user_id = auth.uid()), '0'),
          coalesce((select count(*)::text from public.workout_set_logs where user_id = auth.uid()), '0'),
          coalesce((select sum(weight * greatest(reps, 0))::text from public.workout_set_logs where user_id = auth.uid()), '0'),
          coalesce((select max(created_at)::text from public.workout_set_logs where user_id = auth.uid()), ''),
          coalesce((select max(created_at)::text from public.workout_sessions where user_id = auth.uid()), '')
        ),
        0
      ),
...
revoke all on function public.get_dashboard_revision(text) from public;
grant execute on function public.get_dashboard_revision(text) to authenticated;
...
as $$
  ...
  local_bounds as (
    select
      (date_trunc('day', now() at time zone requested_zone.name) at time zone requested_zone.name) as today_start
    from requested_zone
  ),
  raw_logs as (
    select
      ...
      logs.created_at,
      coalesce(logs.session_id::text, 'legacy:' || to_char(logs.created_at at time zone requested_zone.name, 'YYYY-MM-DD')) as session_key,
  ...
  session_rows as (
    select
      sessions.id::text as session_key,
      sessions.created_at,
    ...
    union all
    select raw.session_key, min(raw.created_at), raw.date_key
    ...
    select raw.session_key, min(raw.created_at), raw.date_key
    from raw_logs as raw
    where raw.session_id is not null
      and not exists (
        select 1 from public.workout_sessions as sessions
        where sessions.id = raw.session_id and sessions.user_id = auth.uid()
      )
    group by raw.session_key, raw.date_key
  ),
  session_metrics as (
    ...
    left join deduped_logs as logs
      ...
      and logs.set_rank = 1
    group by rows.session_key, rows.created_at, rows.date_key
  ),
  period_bounds as (
    select
      requested_zone.name,
  ...
  revision as (
    select public.get_dashboard_revision((select name from requested_zone)) as value
  )
  select jsonb_build_object(
    'revision', (select value from revision),
...
-- Realtime only broadcasts inserts that are performed by the protected
-- admin-announcement function. The table remains RLS-protected for reads.
do $$
begin
  if to_regclass('public.track_announcements') is not null
    ...
    and not exists (
      ...
      where pubname = 'supabase_realtime'
        ...
        and tablename = 'track_announcements'
    ) then
    alter publication supabase_realtime add table public.track_announcements;
  end if;
end;
$$;
